...
n_classes = 6
n_distractors = 2 # Not used, number of candidates = n_classes
message_size = 10
max_conv_length=10
audio_ft_size = 128 # Changeable: image=128, audio=whatever
image_ft_size = 128 
...
batch_size = 32
learning_rate = 1e-4
iterations_per_epoch = 0 # Also not used
optim_type = "RMSprop"
top_k_train = 3
...
img_emb = image_folder + 'init_synth_img.npz'

# PCA
aud_emb = audio_folder + 'init_synth_audio.npz'
...
def train(sender, receiver, baseline_sen, baseline_rec, train_loader, test_loader,
    ...
    for epoch in range(num_epochs):
        ...
        # **Run testing**
        test_loss, test_accuracy, top3, similarity = evaluate(sender, receiver, test_loader)
        similarities.append(similarity)
        testing_accuracies.append(test_accuracy)
        testing_losses.append(test_loss)

    # Plot accuracy and loss
    plot_epoch_times(similarities, save_path='similarities.png')
...
def avg_sen_rec_similarity(sen_feats, rec_feats):
    """
    Given sender and receiver features (lists of message tensors for one conversation),
    compute the average cosine similarity across timesteps.
    """
    sims = []

    for s_msg, r_msg in zip(sen_feats, rec_feats):
        # Flatten to 1D vector
        s_msg = s_msg.detach().cpu().view(-1)
        r_msg = r_msg.detach().cpu().view(-1)

        # Normalize
        s_msg = F.normalize(s_msg, p=2, dim=0)
        r_msg = F.normalize(r_msg, p=2, dim=0)

        # Cosine similarity = dot product
        sim = torch.dot(s_msg, r_msg).item()
        sims.append(sim)
...
def evaluate(sender, receiver, test_loader):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            sim = avg_sen_rec_similarity(sen_feats, rec_feats)
    ...
    avg_similarity = total_similarity / num_convs if num_convs > 0 else 0.0
    avg_loss = total_loss / len(test_loader)
    accuracy = (correct_predictions / total_samples) * 100 if total_samples > 0 else 0
    top3_accuracy = (correct_top3 / total_samples) * 100 if total_samples > 0 else 0

    print(f"Test Loss: {avg_loss:.4f}, Top-1 Accuracy: {accuracy:.2f}%, Top-k Accuracy: {top3_accuracy:.2f}%, Average Similarity: {avg_similarity:.2f}%")
    ...
    return avg_loss, accuracy, top3_accuracy, avg_similarity
